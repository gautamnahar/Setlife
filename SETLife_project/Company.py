...
class Company:
    ...
    def loginform(self):
        ...
        label2 = Label(frame_input, text="User_ID", font=('Goody old style', 20, 'bold'), fg="black", bg="white")
        ...
        label3 = Label(frame_input, text="Password", font=('Goody old style', 20, 'bold'), fg="black", bg="white")
        label3.place(x=30, y=145)
        self.password = Entry(frame_input, show="*",font=("Goody old style", 15, "bold"), bg='white', bd=2)
        ...
        btn2 = Button(frame_input, text="Login", command=self.login, cursor="hand2", font=('times new roman', 15),
                      bg='black', fg='white', bd=0, width=15, height=1)
    ...
    def login(self):
        if self.username_txt.get() == "" or self.password.get() == "":
            ...
            try:
                ...
                cur = con.cursor()
                cur.execute('select * from companylogin where org_id=%s and password=%s',
                            (self.username_txt.get(), self.password.get()))
                row = cur.fetchone()
                if row == None:
                    messagebox.showerror('Error', 'Invalid User_ID or Password', parent=self.root)
                else:
    ...
    def Register(self):
        ...
        Frame_login1 = Frame(self.root, bg="dark blue")
        ...
        label2 = Label(frame_input2, text="User_ID", font=("Goody old style", 20, "bold"), fg='dark blue',
                       bg='white')
        ...
        label3 = Label(frame_input2, text="Password", font=("Goody old style", 20, "bold"), fg='dark blue',
                       bg="white")
        label3.place(x=30, y=145)
        self.entry2 = Entry(frame_input2, show="*", font=("times roman", 15, "bold"), bg='white', bd=2)
        self.entry2.place(x=30, y=195, width=270, height=35)

        label4 = Label(frame_input2, text="Company Name", font=("Goody old style", 20, "bold"), fg='dark blue',
                       bg='white')
        ...
        label5 = Label(frame_input2, text="Confirm Password", font=("Goody old style", 20, "bold"), fg='dark blue',
                       bg='white')
        label5.place(x=330, y=145)
        self.entry4 = Entry(frame_input2, show="*", font=("times new roman", 15, "bold"), bg='white', bd=2)
        self.entry4.place(x=330, y=195, width=270, height=35)

        label5 = Label(frame_input2, text="Contact Number", font=("Goody old style", 20, "bold"), fg='dark blue',
                       bg='white')
        ...
        label6 = Label(frame_input2, text="Email", font=("Goody old style", 20, "bold"), fg='dark blue',
                       bg='white')
        ...
        btn2 = Button(frame_input2, command=self.register, text="Register", cursor="hand2",
                      font=("times new roman", 15), fg="white", bg="dark blue", bd=0, width=15, height=1)
        btn2.place(x=330, y=440)

        btn4 = Button(frame_input2, command=self.verifycation, text="Verify", cursor="hand2",
                      font=("times new roman", 15), fg="white", bg="dark blue", bd=0, width=15, height=1)
    ...
    def register(self):
        if self.entry.get() == "" or self.entry2.get() == "" or self.entry3.get() == "" or self.entry4.get() == "" \
            ...
            try:
                con = pymysql.connect(host="localhost", user="root", password="Mysql@123", database="set_life")
                cur = con.cursor()
                cur.execute("select * from companylogin where email=%s", self.entry6.get())
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error user already exist ,please try with another Email", parent=self.root)
                else:
                    cur.execute("INSERT INTO companylogin VALUES (%s,%s,%s)",
                                (self.entry.get(), self.entry6.get(), self.entry2.get()))
                    cur.execute("INSERT INTO company (org_id,name,email,contact) VALUES (%s,%s,%s,%s)",
                                (self.entry.get(), self.entry3.get(), self.entry6.get(), self.entry5.get()))
                    con.commit()
                    con.close()
